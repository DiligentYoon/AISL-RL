...
class PFGOATBaseEnv(Env):
    # Load config file
    cfg: PFGOATBaseEnvCfg

    def __init__(self, cfg: PFGOATBaseEnvCfg, render_mode: str | None = None, **kwargs):
        ...
        self.thigh_joint_ids, _ = self._robot.find_joints(["thigh_.*"])
        self.knee_joint_ids, _ = self._robot.find_joints(["knee_.*"])
        # Link Ids
        self.hip_link_ids, _ = self._robot.find_bodies(["hip_.*"])
        self.thigh_link_ids, _ = self._robot.find_bodies(["thigh_.*"])
        self.calf_link_ids, _ = self._robot.find_bodies(["calf_.*"])
        # Contact Link Ids
        self.contact_hip_link_ids, _ = self.contact_sensors.find_bodies(["hip_.*"])
        self.contact_thigh_link_ids, _ = self.contact_sensors.find_bodies(["thigh_.*"])
        self.contact_calf_link_ids, _ = self.contact_sensors.find_bodies(["calf_.*"])
