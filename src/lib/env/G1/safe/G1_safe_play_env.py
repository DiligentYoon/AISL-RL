...
class G1SafePlayEnv(G1BaseEnv):
    cfg: G1SafeEnvCfg

    def __init__(self, cfg: G1SafeEnvCfg, render_mode: str | None = None, **kwargs):
        super().__init__(cfg, render_mode, **kwargs)
        # Safety policy space (observation, state, action)
        single_safe_observation_space = spec_to_gym_space(self.cfg.safe_observation_space)
        single_safe_state_space = spec_to_gym_space(self.cfg.safe_state_space)
        single_safe_action_space = spec_to_gym_space(self.cfg.safe_action_space)

        self.safe_observation_space = gym.vector.utils.batch_space(single_safe_observation_space, self.num_envs)
        self.safe_state_space = gym.vector.utils.batch_space(single_safe_state_space, self.num_envs)
        self.safe_action_space = gym.vector.utils.batch_space(single_safe_action_space, self.num_envs)
