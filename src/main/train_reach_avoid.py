...
parser.add_argument("--checkpoint", type=str, default="logs/g1_recovery/2026-03-13_00-05-45_mappo/agent_72000.pt", help="Path to model checkpoint.")
...
def main():
    ...
    while simulation_app.is_running() and timestep <= cfg["train"]["timesteps"]:
        ...
        if timestep % ra_buffer.buffer_size == 0:
            per_update_value_loss = float(np.mean(CLI_step_reward_means)) if len(CLI_step_reward_means) else float("nan")
